...
namespace Vulkan {
    ...
    void Image::transitionImageLayout([[maybe_unused]] VkFormat format, const VkImageLayout oldLayout, const VkImageLayout newLayout, const uint32_t mipLevels) const {
    ...
    void Image::copyBufferToImage(const VkBuffer buffer, const uint32_t width, const uint32_t height) const {
        const auto commandBuffer = m_Device->beginSingleTimeCommands();
    ...
    void Image::generateMipmaps(const VkFormat imageFormat, const int32_t texWidth, const int32_t texHeight, const uint32_t mipLevels) const {
        // Check if image format supports linear blitting
        VkFormatProperties formatProperties;
        vkGetPhysicalDeviceFormatProperties(Context::get()->getPhysicalDevice()->get(), imageFormat, &formatProperties);
